...
function fileUpload (fields, files, filePath, fileId='filetoupload') {
  ...
  if (!fs.existsSync(path.join(__dirname, '../data/issues', fields.id, filePath))) {
    fs.mkdirSync(path.join(__dirname, '../data/issues', fields.id, filePath), { recursive: true });
  }
  let newpath = path.join(__dirname, '../data/issues', fields.id, filePath, files[fileId].name);
  try {
